...
describe(basename(__filename), () => {
  ...
  it('the npm page should be opened when keyword was set', async () => {
    ...
    expect(openMock).toHaveBeenCalledWith(
      `${npmWebsite}/search?q=to-where-cli`
    );
  });
  ...
  it('the npm page should be opened when keyword was set && -c', async () => {
    ...
    expect(openMock).toHaveBeenCalledWith(
      `${npmWebsite}/package/to-where-cli?activeTab=code`
    );
  });
  ...
  it('the npm page should be opened when keyword was set && -d', async () => {
    ...
    expect(openMock).toHaveBeenCalledWith(
      `${npmWebsite}/package/to-where-cli?activeTab=dependencies`
    );
  });

  it('the npm page should be opened when keyword was set && -r', async () => {
    const program = createProgram();
    ...
    expect(openMock).toHaveBeenCalledWith(
      `https://npm.runkit.com/to-where-cli`
    );
  });
  ...
  it('the npm page should be opened when keyword was set && -v', async () => {
    ...
    expect(openMock).toHaveBeenCalledWith(
      `${npmWebsite}/package/to-where-cli?activeTab=versions`
    );
  });
});
